...
export default function BriefingPage() {
  const b = getPriceWatch();
  const cpiSnap = getLatestSnapshot().map((c) => ({ name: c.name, dev: c.dev }));
  ...
  return (
    <div className="max-w-4xl mx-auto w-full px-5 py-8 print:py-0 print:px-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 print:hidden">
        <div>
          <span className="eyebrow">Briefing artifact</span>
          <h1 className="mt-1 text-2xl font-semibold font-display text-ink">Price Watch — Weekly Brief</h1>
        </div>
        <BriefingActions />
      </div>
      ...
      <article className="memo rounded-sm bg-white border border-[rgba(18,32,25,0.14)] p-7 sm:p-9 print:border-0 print:p-0">
        ...
        <header className="flex items-start justify-between gap-4 pb-5 border-b border-[rgba(18,32,25,0.14)]">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <p className="font-display text-xl font-semibold text-ink">Price Watch</p>
              <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-slate-500">Weekly price situation · Office of the Minister</p>
            </div>
          </div>
          <div className="text-right font-mono text-[10px] text-slate-500 leading-snug">
            AS OF<br />{b.asOf}<br /><span className="text-slate-400">{b.period}</span>
          </div>
        </header>

        {/* Executive summary */}
        <section className="mt-6">
          <h2 className="font-mono text-[10px] uppercase tracking-[0.18em] text-amber-600">Executive summary</h2>
        ...
        <section className="mt-7 grid sm:grid-cols-2 gap-6">
          <div>
            <h3 className="font-semibold text-ink text-sm">CPI category deviation from base 100</h3>
            <div className="mt-2"><CpiDeviationChart data={cpiSnap} height={300} /></div>
          </div>
          <div>
            <h3 className="font-semibold text-ink text-sm">Affordability — items by price band</h3>
        ...
        <section className="mt-7 rounded-sm border border-amber-500/30 bg-amber-50/50 p-4 break-inside-avoid">
          <h3 className="font-mono text-[10px] uppercase tracking-[0.18em] text-amber-700">Watch list — for decision</h3>
        ...
        <footer className="mt-7 pt-4 border-t border-[rgba(18,32,25,0.14)] text-[11px] text-slate-400 leading-relaxed">
          Auto-generated from the Lebanon Prices Intelligence Unit datasets (non-core daily CPI; retail shelf snapshot {b.snapshotDates?.join(" & ")}; import-origin signal). Figures are cross-sectional and directional — verify before any decision. Market-level only; outlet names withheld.
        </footer>
      </article>
    </div>
  );
