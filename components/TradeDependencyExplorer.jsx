...
export default function TradeDependencyExplorer({ concentration, countries, critical, locale = "en" }) {
  ...
  const legend = [
    ["≥80%", tr("trade.legendMono"), "#c2152e"],
    ["50–79%", tr("trade.legendConc"), "#9a7b3f"],
    ["25–49%", tr("trade.legendMod"), "#1f5c3c"],
    ["<25%", tr("trade.legendDiv"), "#3b4a5a"],
  ];
    ...
    <div className="grid lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 rounded-2xl border border-paper/10 bg-ink overflow-hidden">
        <div className="px-5 pt-5 pb-3">
          <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-amber-400">{tr("trade.networkEyebrow")}</p>
          <h2 className="mt-1 text-xl font-semibold text-paper flex items-center gap-2">
            {lens === "all" ? (
              <>🌐 {tr("trade.networkTitle")}</>
            ) : (
              <>📦 {lens} <span className="text-paper/50 text-base">{ar ? "← لبنان" : "→ Lebanon"}</span></>
            )}
          </h2>
          <p className="text-xs text-paper/55 mt-1">
            {lens === "all"
              ? tr("trade.networkSub")
              : `${tr("trade.countrySub")} ${lens} ${tr("trade.countrySupplies")} ${lens}${tr("trade.countryGrip")}`}
          </p>
        </div>
      ...
      <div className="space-y-6">
        {country && (
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-amber-600">{tr("trade.route")}</p>
            <p className="mt-1 font-semibold text-ink">{country.route}</p>
            <div className="mt-3 flex flex-wrap gap-1.5">
              {(country.chokepoints.length ? country.chokepoints : [tr("trade.noChoke")]).map((ch) => (
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-center">
              <div className="rounded-lg bg-slate-50 py-2">
                ...
                <p className="text-[10px] text-slate-500">{tr("trade.monopolies")}</p>
              </div>
              <div className="rounded-lg bg-slate-50 py-2">
                <p className="text-lg font-bold text-ink">{country.dominantCategories}</p>
                <p className="text-[10px] text-slate-500">{tr("trade.catsOver50")}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 min-h-[160px]">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-slate-500">
            {node ? tr("trade.selected") : tr("trade.hover")}
          </p>
          {node ? (
            <>
              ...
              <p className="text-xs text-slate-500">
                {lens === "all"
                  ? <>{tr("trade.from")} <span className="font-medium text-ink">{node.source}</span> · {node.size.toLocaleString()} {tr("trade.items")}</>
                  : <>{ar ? <>قبضة <span className="font-medium text-ink">{lens}</span></> : <><span className="font-medium text-ink">{lens}</span>&apos;s grip</>} · {node.size.toLocaleString()} {tr("trade.items")}</>}
              </p>
              {lens === "all" && node.top3 && (
                <div className="mt-3 space-y-1.5">
                  <p className="text-[10px] uppercase tracking-wide text-slate-400 font-mono">{tr("trade.topSources")}</p>
                  {node.top3.map((s) => (
                    <div key={s.name} className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-mono text-slate-500 w-10 text-right rtl:text-left">{s.sharePct}%</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <p className="mt-2 text-sm text-slate-500 leading-relaxed">{tr("trade.hoverHelp")}</p>
          )}
        </div>

        {/* Critical dependencies */}
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-cedar">{tr("trade.critical")}</p>
          <p className="text-xs text-slate-500 mt-0.5">{tr("trade.criticalSub")}</p>
        ...
        <div className="rounded-2xl border border-brand-100 bg-brand-50/60 p-5">
          <p className="font-semibold text-ink">{tr("trade.advisor")}</p>
          <p className="text-xs text-slate-600 mt-1">{tr("trade.advisorSub")}</p>
          <button
            type="button"
