  ...
  if (data.meta?.usingEqualWeights) {
    logWarn(
      "getCategoryGaps(): category gapPct values are UNWEIGHTED (equal weights) — " +
        "real NonCoreItemBreakdown.Share from Azure SQL was not available when data/basket_prices.json was built."
    );
  }
  return rows;
}
...
export function getBasketChainContext() {
  ...
  L.push(
    `- Dearest chain by category (weighted average of item gaps by CPI importance weight${
      data.meta?.usingEqualWeights ? " — NOTE: equal-weighted fallback, real NonCoreItemBreakdown.Share not yet wired in" : ""
...
export function getBasketPriceDispersion(topN = 10) {
  const rows = [...(data.items || [])]
    ...
    .map((it) => ({
      ...
      unit: it.unit,
      geoMean: r2(it.geoMeanUnitPrice),
      gapPct: it.gapPct,
      nChains: it.nChains,
      byChain: Object.fromEntries(Object.entries(it.unitByChain || {}).map(([ch, up]) => [ch, r2(up)])),
      dearChain: it.dearChain,
      cheapChain: it.cheapChain,
    }));
  if (!rows.length) {
    logWarn("getBasketPriceDispersion(): no items with a valid gap were found.");
  }
  return { top: rows, comparedItems: (data.items || []).length };
}
