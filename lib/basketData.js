...
export function getBasketChainContext() {
  ...
  const spreads = [...data.cheapestByCategory].sort((a, b) => b.spreadPct - a.spreadPct).slice(0, 8);
  L.push(
    `- Cheapest vs dearest chain by category (widest gaps first): ${spreads
      .map((s) => `${s.category}: cheapest ${s.cheapest} $${s.cheapestPrice} vs dearest ${s.dearest} $${s.dearestPrice} (+${s.spreadPct}%)`)
      .join("; ")}.`
  );
  L.push(
    `- Category median by chain: ${data.categories
      .slice(0, 10)
      .map((c) => `${c.name} [${Object.entries(c.byChain).map(([ch, p]) => `${ch} $${p}`).join(", ")}]`)
      .join("; ")}.`
  );
  return L.join("\n");
}
...
export function getBasketItemGapContext(n = 30) {
  const rows = (data.items || [])
    .map((it) => {
      const chains = Object.entries(it.byChain || {}).filter(([, v]) => v && v.price > 0);
      if (chains.length < 2) return null;
      chains.sort((a, b) => a[1].price - b[1].price);
      const [cheapCh, cheap] = chains[0];
      const [dearCh, dear] = chains[chains.length - 1];
      const gap = Math.round(((dear.price - cheap.price) / cheap.price) * 100);
      return {
        item: it.cpi_item, category: it.category, gap, nChains: chains.length,
        cheapCh, cheapP: cheap.price, cheapProd: cheap.product,
        dearCh, dearP: dear.price, dearProd: dear.product,
      };
    })
    .filter(Boolean)
  ...
  const L = [];
  L.push(
    `Per-ITEM cross-chain prices (the SAME CPI item priced at each named chain, with each chain's actual product; ${(data.items || []).length} items tracked; top ${rows.length} by gap — use these for true per-product, per-supermarket comparison):`
  );
  for (const r of rows) {
    L.push(
      `- ${r.item} (${r.category}): cheapest ${r.cheapCh} $${r.cheapP} [${r.cheapProd}] vs dearest ${r.dearCh} $${r.dearP} [${r.dearProd}] — +${r.gap}%${r.nChains < 3 ? ` (only ${r.nChains} of 3 chains carry it)` : ""}.`
    );
  }
  return L.join("\n");
}
...
export function getForensicWatch({ topCats = 6, topItems = 10 } = {}) {
  ...
  const cheapestChain = chainList.reduce((a, b) => (b.medianPrice < a.medianPrice ? b : a), chainList[0] || { medianPrice: 0 });
  const dearestCount = {};
  chainList.forEach((c) => { dearestCount[c.name] = 0; });
  (data.cheapestByCategory || []).forEach((c) => { if (dearestCount[c.dearest] != null) dearestCount[c.dearest]++; });
  ...
  const chains = chainList
    .map((c) => ({
      ...
      items: c.items,
      premiumVsCheapest: cheapestChain.medianPrice ? Math.round(((c.medianPrice - cheapestChain.medianPrice) / cheapestChain.medianPrice) * 100) : 0,
      dearestInCats: dearestCount[c.name] || 0,
    }))
    .sort((a, b) => b.dearestInCats - a.dearestInCats || b.medianPrice - a.medianPrice);

  const categories = [...(data.cheapestByCategory || [])]
    .sort((a, b) => b.spreadPct - a.spreadPct)
    .slice(0, topCats)
    .map((c) => ({ category: c.category, dearest: c.dearest, dearestPrice: c.dearestPrice, cheapest: c.cheapest, cheapestPrice: c.cheapestPrice, gap: c.spreadPct }));

  const items = (data.items || [])
    .map((it) => {
      const chs = Object.entries(it.byChain || {}).filter(([, v]) => v && v.price > 0);
      if (chs.length < 2) return null;
      chs.sort((a, b) => a[1].price - b[1].price);
      const [cheapCh, cheap] = chs[0];
      const [dearCh, dear] = chs[chs.length - 1];
      const gap = Math.round(((dear.price - cheap.price) / cheap.price) * 100);
      return { item: it.cpi_item, category: it.category, cheapCh, cheapP: cheap.price, dearCh, dearP: dear.price, dearProd: dear.product, gap, nChains: chs.length };
    })
    .filter(Boolean)
    .sort((a, b) => b.gap - a.gap)
    .slice(0, topItems);
  ...
  return {
    ...
    totalCats: (data.cheapestByCategory || []).length,
    chains,
    categories,
    items,
  };
