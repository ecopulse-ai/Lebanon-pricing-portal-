...
export function getBasketProductDispersion(topN = 20) {
  ...
  const rows = (data.products || [])
    .map((p) => {
      ...
      const priceSpan = Math.abs((byChain[dearChain] ?? 0) - (byChain[cheapChain] ?? 0));
      return {
        item: p.productName,
        cpiItem: p.cpi_item,
        category: p.category,
    ...
    .filter(Boolean)
    .sort((a, b) => b.priceSpan - a.priceSpan) // longest visible bar first, NOT
      // biggest percentage gap -- those aren't the same ranking: a large %
      // gap on a cheap item can be a short bar, and vice versa. This is what
      // actually determines bar length on the chart's dollar-denominated axis.
    .slice(0, topN);
  if (!rows.length) {
...
export function getForensicWatch({ topItems = 10 } = {}) {
  ...
  const items = [...(data.flaggedItems || [])]
    ...
    .slice(0, topItems)
    .map((it) => ({
      // Full raw fields -- what app/products/page.js's item-gap table (next
      // to All Categories) actually reads. Added after a real production
      // crash: that table expected these directly on `items`, but this
      // function was only returning the aliased/renamed fields below for
      // the (older) advice-text usage -- `unitByChain` wasn't here at all,
      // so `Object.keys(r.unitByChain)` threw on every row.
      code: it.code,
      cpi_item: it.cpi_item,
      unitByChain: it.unitByChain,
      dearChain: it.dearChain,
      cheapChain: it.cheapChain,
      gapPct: it.gapPct,
      // Aliased/shortened fields -- kept for the advice-text paragraph,
      // which reads it0.item / it0.gap / it0.dearCh / it0.unit.
      item: it.cpi_item,
