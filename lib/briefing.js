    ...
    const sections = [
      {
        ...
        body: `أغلق المؤشّر غير الأساسي الأسبوع عند ${cpi.cpi} (أساس 100)، و${dir} ${sign(cd)}${cd}% في اليوم. الغذاء إجمالاً عند ${cpi.foodOverall} (${sign(cpi.foodDoD)}${cpi.foodDoD}% يومياً) ومؤشّر الغاز عند ${cpi.gas} (${sign(cpi.gasDoD)}${cpi.gasDoD}% يومياً).`,
        bullets: [
