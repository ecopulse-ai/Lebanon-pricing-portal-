...
export async function getPriceWatch(locale = "en") {
  ...
  // NOTE: fw.chains is ordered by basket coverage (item count), not premium —
  // rank explicitly here, and skip chains with no comparable items yet
  // (avgPremiumPct === null), since those aren't "0% premium", they're "no data".
  const fw = getForensicWatch({ topItems: 5 });
  const rankedChains = [...fw.chains]
    .filter((c) => c.avgPremiumPct != null)
    .sort((a, b) => b.avgPremiumPct - a.avgPremiumPct);
  const topChain = rankedChains[0] || { name: "—", avgPremiumPct: 0, dearestItems: 0, itemsCompared: 0 };
  const topItem = fw.items[0] || null;
  const topCat = fw.categoryGaps.find((c) => !c.insufficientData) || null;
  ...
  if (ar) {
    ...
    const sections = [
      ...
      },
      {
        ...
        body: `بمقارنة الصنف نفسه بين المتاجر على وحدة التعبئة الفعلية لكل صنف (${fw.comparedItems} صنفاً قابلاً للمقارنة، متوفرة لدى كل منفذ)، ${topChain.name} هي الأغلى — علاوة وسطية +${topChain.avgPremiumPct}% لكل وحدة فوق أرخص منافس، والأعلى في ${topChain.dearestItems} من ${topChain.itemsCompared} صنفاً.`,
        bullets: [
          ...
          topCat ? `فئة للمراجعة: ${topCat.category} — الأغلى ${topCat.dearest} مقابل ${topCat.cheapest} (فجوة مرجّحة +${topCat.gapPct}%).` : "—",
        ],
      },
      {
  ...
  const sections = [
    ...
    },
    {
      ...
      body: `Comparing the same item across supermarkets on each item's actual pack unit (${fw.comparedItems} comparable items, present at every chain), ` +
      ...
      bullets: [
        ...
        topCat ? `Category to review: ${topCat.category} — dearest ${topCat.dearest} vs ${topCat.cheapest} (weighted +${topCat.gapPct}%).` : "—",
      ],
    },
    {
