...
const ROUTE = {
  ...
  Ireland: { bloc: "Europe & UK", route: "Atlantic → Med", choke: [] },
  "United Kingdom": { bloc: "Europe & UK", route: "Atlantic → Med", choke: [] },
  Belgium: { bloc: "Europe & UK", route: "Atlantic → Med", choke: [] },
  Netherlands: { bloc: "Europe & UK", route: "Atlantic → Med", choke: [] },
  Germany: { bloc: "Europe & UK", route: "Atlantic → Med", choke: [] },
  Denmark: { bloc: "Europe & UK", route: "Atlantic → Med", choke: [] },
  Italy: { bloc: "Europe & UK", route: "Across Mediterranean", choke: [] },
  Czechia: { bloc: "Europe & UK", route: "Atlantic → Med", choke: [] },
  ...
  "United States": { bloc: "Americas", route: "Atlantic → Med", choke: [] },
  Brazil: { bloc: "Americas", route: "Atlantic → Med", choke: [] },
...
export function buildMarket(rows) {
  ...
  for (const row of rows) {
    ...
    // Lebanon is the country under analysis — domestically sourced goods are
    // self-sufficiency, NOT imports. Exclude them from every import-dependency
    // metric (origins, per-category sourcing, supplier countries, blocs,
    // chokepoints), so shares reflect foreign imports only.
    if (o && o !== "Lebanon") {
      inc(origins, o);
      originRows++;
