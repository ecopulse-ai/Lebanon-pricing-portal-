...
export async function getUnitPriceWatch({ topN = 10, minVariants = 3, minListings = 12 } = {}) {
  ...
  for (const p of products.products) {
    ...
    // Count/length goods (diapers, wipes) are not meaningfully priced per 100g/ml.
    if (/\b(?:diaper|diapers|nappy|nappies|wipe|wipes|pull ?ups?)\b/.test(core)) continue;
  ...
  for (const g of goods.values()) {
    ...
    if (r2(lo) < 0.05 || r2(hi) <= r2(lo)) continue; // drop near-zero / degenerate unit prices
    const spreadPct = Math.round(((hi - lo) / lo) * 100);
